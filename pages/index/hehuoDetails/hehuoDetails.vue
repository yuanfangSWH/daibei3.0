	...
	<view v-show="pageshow">
		<view class="hehuo-nav01">
			<view class="hehuo-je">
				...
				<view class="hehuo-a03">
					<view class="hehuo-b01">
						<image class="qian-icon" style="margin-left: 132upx;" src="../../../static/qian-icon.png"></image><text class="hehuo-t01">返佣{{infobox.loan_pass_fanyong}}</text>
					</view>
					<view class="hehuo-b01">
						<image class="qian-icon" src="../../../static/time-icon.png"></image><text style="margin-right: 100upx;" class="hehuo-t01">{{infobox.loan_pass_end}}</text>
					</view>
				</view>
			</view>
			<!-- <view class="xq-nav02">
			...
			<view class="sq-yaoqiu">
				...
				<view class="yaoqiu-title">返佣说明</view>
				<view class="yaoqiu-list">
					<view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			dataFun: function() {
				...
				api.apiPost('/user/teamext', '', '', 'get', (status, res, code) => {
					if (res.code == 0) {
						...
						var wxtitle = _this.infobox.loan_name;;
			...
			loanapply: function() {
				uni.navigateTo({
					url: '/pages/index/loanApply/loanApply?key=' + this.key + '&fxtitle=' + encodeURIComponent(this.loanname) +
						'&icon=' + encodeURIComponent(this.pIcon)
				})
			},
			loanTuig: function() {
				uni.navigateTo({
					url: '/pages/index/loanposter/loanposter?id=' + this.productID
				})
			},
...
<style scoped>
	...
	.hehuo-je {
		...
		background-size: 716upx 246upx;
		box-shadow: 0px 6upx 18upx 0px rgba(27, 115, 245, 0.5);
		border-radius: 20upx;
	}
